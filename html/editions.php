...
#	$res = $res."</table>";
	...
	$res = "";
	foreach ($sql->query($query) as $row) {
		$res = $res.'<a href="../plain/passage.php?urn='.$row['urn'].'&deletexml&nl">Passage</a> '.'<a href="../html/structure.php?urn='.$row['urn'].'">Structure</a> '.$row['urn'].' '.$row['title'].' '.$row['year'].' '.$row['author'].'<br/>';
	}
